...
static void LoadBattleEnvironmentGfx(u16 environment)
{
    ...
    {
    case TIME_MORNING:
        paletteToLoad = gBattleEnvironmentInfo[environment].paletteDawn;
        if (paletteToLoad == NULL)
            paletteToLoad = gBattleEnvironmentInfo[environment].palette;
        break;
    case TIME_EVENING:
        paletteToLoad = gBattleEnvironmentInfo[environment].paletteEvening;
        if (paletteToLoad == NULL)
            paletteToLoad = gBattleEnvironmentInfo[environment].palette;
        break;
    case TIME_NIGHT:
        paletteToLoad = gBattleEnvironmentInfo[environment].paletteNight;
        if (paletteToLoad == NULL)
            paletteToLoad = gBattleEnvironmentInfo[environment].palette;
        break;
    case TIME_DAY:
    ...
    case 5:
        {
            ...
            {
            case TIME_MORNING:
                paletteToLoad = gBattleEnvironmentInfo[terrain].paletteDawn;
                if (paletteToLoad == NULL)
                    paletteToLoad = gBattleEnvironmentInfo[terrain].palette;
                break;
            case TIME_EVENING:
                paletteToLoad = gBattleEnvironmentInfo[terrain].paletteEvening;
                if (paletteToLoad == NULL)
                    paletteToLoad = gBattleEnvironmentInfo[terrain].palette;
                break;
            case TIME_NIGHT:
                paletteToLoad = gBattleEnvironmentInfo[terrain].paletteNight;
                if (paletteToLoad == NULL)
                    paletteToLoad = gBattleEnvironmentInfo[terrain].palette;
                break;
            case TIME_DAY:
