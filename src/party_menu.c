...
static void UpdatePartySelectionSingleLayout(s8 *slotPtr, s8 movementDir)
{
    ...
    case MENU_DIR_UP:
        if (*slotPtr == PARTY_SIZE + 1)
        {
            if (sPartyMenuInternal->chooseHalf)
                *slotPtr = PARTY_SIZE;
            else if (gPartiesCount[partyTrainer] <= 1)
                *slotPtr = 0;
            else
                *slotPtr = gPartiesCount[partyTrainer] - 1 - (gPartiesCount[partyTrainer] % 2);
        }
        ...
        else if (*slotPtr >= gPartiesCount[partyTrainer] && *slotPtr < PARTY_SIZE)
        {
            *slotPtr = PARTY_SIZE + 1;
        }
        else
    ...
    case MENU_DIR_DOWN:
        ...
        {
            if (sPartyMenuInternal->chooseHalf && (*slotPtr == gPartiesCount[partyTrainer] - 1 || *slotPtr == gPartiesCount[partyTrainer] - 2))
                *slotPtr = PARTY_SIZE;
            else
                *slotPtr = PARTY_SIZE + 1;
        }
    ...
    case MENU_DIR_LEFT:
        ...
        {
            (*slotPtr)--;
            if (*slotPtr >= gPartiesCount[partyTrainer])
                *slotPtr = PARTY_SIZE + 1;
        }
    break;
    case MENU_DIR_RIGHT:
        if (*slotPtr == PARTY_SIZE + 1)
            *slotPtr = 0;
        else if (sPartyMenuInternal->chooseHalf && *slotPtr == gPartiesCount[partyTrainer] - 1)
            *slotPtr = PARTY_SIZE;
        else if (*slotPtr + 1 < gPartiesCount[partyTrainer])
            (*slotPtr)++;
        else
            *slotPtr = PARTY_SIZE + 1;
    break;
...
static void Task_PartyMenuModifyHP(u8 taskId)
{
    ...
    SetMonData(&party[tPartyId], MON_DATA_HP, &tHP);
    DisplayPartyPokemonHPCheck(&party[tPartyId], &sPartyMenuBoxes[tPartyId], 1);
    DisplayPartyPokemonMaxHPCheck(&party[tPartyId], &sPartyMenuBoxes[tPartyId], 1);
    DisplayPartyPokemonHPBarCheck(&party[tPartyId], &sPartyMenuBoxes[tPartyId]);
...
static void CreateCancelConfirmWindows(bool8 chooseHalf)
{
     ...
     && gPartyMenu.menuType != PARTY_MENU_TYPE_MULTI_FULL_SHOWCASE)
    {
        ...
        else if (gPartyMenu.layout != PARTY_LAYOUT_MULTI)
        {
