...
void UpdateTallDarkGrassFieldEffect(struct Sprite *sprite)
{
    u8 metatileBehavior;
    u8 localId;
    u8 objectEventId;
...
u32 FldEff_JumpDarkTallGrass(void)
{
    u8 spriteId;
...
u8 FindTallDarkGrassFieldEffectSpriteId(u8 localId, u8 mapNum, u8 mapGroup, s16 x, s16 y)
{
    u8 i;
    for (i = 0; i < MAX_SPRITES; i ++)
    {
        ...
        {
            struct Sprite *sprite = &gSprites[i];
            if (sprite->callback == UpdateTallDarkGrassFieldEffect
                && (x == sprite->sX && y == sprite->sY)
                && localId == (u8)(sprite->sLocalId)
                && mapNum == (sprite->sMapNum & 0xFF)
                && mapGroup == sprite->sMapGroup)
...
void UpdateLongDarkGrassFieldEffect(struct Sprite *sprite)
{
    u8 metatileBehavior;
    u8 localId;
    u8 objectEventId;
...
u32 FldEff_JumpLongDarkGrass(void)
{
    u8 spriteId;
