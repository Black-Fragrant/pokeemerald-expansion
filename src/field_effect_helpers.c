...
u32 FldEff_TallGrassDark(void)
{
    ...
    s16 y = gFieldEffectArguments[1];
    SetSpritePosToOffsetMapCoords(&x, &y, 8, 8);
    spriteId = CreateSpriteAtEnd(gFieldEffectObjectTemplatePointers[FLDEFFOBJ_TALL_GRASS_DARK], x, y, 0);
    if (spriteId != MAX_SPRITES)
    {
        struct Sprite *sprite = &gSprites[spriteId];
        sprite->coordOffsetEnabled = TRUE;
...
void UpdateTallGrassFieldEffectDark(struct Sprite *sprite)
{
    ...
    if (TryGetObjectEventIdByLocalIdAndMap(localId, mapNum, mapGroup, &objectEventId)
     || !MetatileBehavior_IsTallGrassDark(metatileBehavior)
     || (sprite->sObjectMoved && sprite->animEnded))
    {
        FieldEffectStop(sprite, FLDEFF_TALL_GRASS_DARK);
    }
    else
    {
        struct ObjectEvent *objectEvent = &gObjectEvents[objectEventId];
...
u8 FindTallGrassFieldEffectSpriteIdDark(u8 localId, u8 mapNum, u8 mapGroup, s16 x, s16 y)
{
    u8 i;
    for (i = 0; i < MAX_SPRITES; i++)
    {
        ...
        {
            struct Sprite *sprite = &gSprites[i];
            if (sprite->callback == UpdateTallGrassFieldEffectDark
             && x == sprite->sX && y == sprite->sY
             && localId == (u8)(sprite->sLocalId)
             && mapNum == (sprite->sMapNum & 0xFF)
             && mapGroup == sprite->sMapGroup)
