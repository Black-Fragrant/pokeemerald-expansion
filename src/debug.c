...
#define SOUND_LIST_BGM              \
	...
	X(MUS_BW_VS_CHEREN_BIANCA)      \
	X(MUS_BW_VS_WILD_STRONG)        \
	X(MUS_BW_MOVE_DELETED)          \
	X(MUS_BW_FOLLOW_ME)             \
	X(MUS_BW_VS_BATTLE_SUBWAY_BOSS) \
	X(MUS_BW_APPEARS_PLASMA)        \
	X(MUS_BW_GYM)                   \
	X(MUS_BW_UNWAVERING_EMOTIONS)   \
	X(MUS_BW_THE_POKEMON_1)         \
	X(MUS_BW_TITLE_SCREEN)          \
	X(MUS_BW_UNDELLA_TOWN_SUMMER)   \
	X(MUS_BW_LACUNOSA_TOWN)         \
	X(MUS_BW_CYCLING)               \
	X(MUS_BW_SPOTTED_SCIENTIST)     \
	X(MUS_BW_SPOTTED_LASS)          \
	X(MUS_BW_SPOTTED_YOUNGSTER)     \
	X(MUS_BW_HALL_OF_FAME)          \
	X(MUS_BW_DREAM_YARD)            \
	X(MUS_BW_GENERAL_CAVE)          \
	X(MUS_BW_STRANGE_HOUSE)         \
	X(MUS_BW_CREDITS_ROLL)          \
	X(MUS_BW_ROUTE_19)              \
	X(MUS_BW_ROUTE_4)               \
	X(MUS_BW_ROUTE_2)               \
	X(MUS_BW_ROUTE_6)               \
	X(MUS_BW_VICTORY_ROAD)          \
	X(MUS_BW_VS_GYM_LEADER_1)       \
	X(MUS_BW_VS_TRAINER_1)          \
	X(MUS_BW_VS_WILD_1)             \
	X(MUS_BW_VS_ALDER)              \
	X(MUS_BW_ASPERTIA_CITY)         \
	X(MUS_BW_JUNIPER_LAB)           \
	X(MUS_BW_JUNIPER)               \
	X(MUS_BW_POKEMON_CENTER)        \
	X(MUS_BW_045)                   \
	X(MUS_BW_SURFING)               \
	X(MUS_BW_DRAGONSPIRAL_TOWER)    \
	X(MUS_BW_COLD_STORAGE)          \
	X(MUS_BW_VERBANK_CITY)          \
	X(MUS_BW_VICTORY_TRAINER)       \
	X(MUS_BW_VICTORY_WILD)          \
	X(MUS_BW_GYM_LEADER)            \
	X(MUS_BW_CASTELIA_CITY)         \
	X(MUS_BW_FLOCCESY_TOWN)         \
	X(MUS_BW_HUGH)                  \
	X(MUS_BW_HUGH_2)                \
	X(MUS_BW_DEX_RATING)            \
	...
	X(MUS_BW_CAUGHT_2)              \
	X(MUS_BW_GAMEFREAK)             \
	X(MUS_BW_VICTORY_WILD_2)        \
	X(MUS_BW_MISSION_RECEIVED)      \
	X(MUS_BW_NS_CASTLE)             \
	X(MUS_BW_TUBELINE_BRIDGE)       \
	X(MUS_BW_SKYARROW_BRIDGE)       \
	X(MUS_BW_ICIRRUS_CITY)          \
	X(MUS_BW_DRIFTVEIL_DRAWBRIDGE)  \
	X(MUS_BW_RELIC_CASTLE)          \
	X(MUS_BW_ROUTE_10)              \
	X(MUS_BW_MARVELOUS_BRIDGE)      \
	X(MUS_BW_STRIATON_CITY)         \
	X(MUS_BW_DRIFTVEIL_CITY)        \
	X(MUS_BW_MISTRALTON_CITY)       \
	X(MUS_BW_VS_ZEKROM)             \
	X(MUS_BW_VS_RESHIRAM)           \
	X(MUS_BW_VS_LEGENDARY)          \
	X(MUS_BW_SPOTTED_ACE_TRAINER)   \
	X(MUS_BW_POKEMON_LEAGUE)        \
	X(MUS_BW_VS_PWC_FINAL)          \
	X(MUS_BW_FAREWELL)              \
	X(MUS_BW_PASSAGE_TUNNEL)        \
	X(MUS_BW_VS_TRAINER_SUBWAY)     \
	X(MUS_BW_VS_N)                  \
	X(MUS_BW_VS_GHETSIS_1)          \
	...
	X(MUS_BW_SPOTTED_PARASOL_LADY)  \
	X(MUS_BW_SPOTTED_HIKER)         \
	X(MUS_BW_ALDER)                 \
	X(MUS_BW_SPOTTED_BUSINESSMAN)   \
	X(MUS_BW_SPOTTED_CYCLIST)       \
	X(MUS_BW_SPOTTED_GENTLEMEN)     \
	X(MUS_BW_N)                     \
	X(MUS_BW_SPOTTED_POKEFAN)       \
	X(MUS_BW_SPOTTED_PSYCHIC)       \
	X(MUS_BW_SPOTTED_ROUGHNECK)     \
	...
	X(MUS_BW_VS_N_DECISIVE_BATTLE)  \
	X(MUS_BW_OPELUCID_CITY_BLACK)   \
	X(MUS_BW_OPELUCID_CITY_WHITE)   \
	X(MUS_BW_SPAWN_NUVEMA_TOWN)     \
	X(MUS_BW_ADVENTURE_TOGETHER)    \
	X(MUS_BW_TROUBLE)               \
	X(MUS_BW_PLOT_PLASMA)           \
	X(MUS_BW_CYNTIA_INTRO)          \
	X(MUS_BW_VS_CYNTIA)             \
	X(MUS_BW_GHETSIS_AMBITIONS)     \
	X(MUS_BW_VS_WILD_2)             \
	X(MUS_BW_VS_TRAINER_2)          \
	X(MUS_BW_NUVEMA_TOWN)           \
	X(MUS_BW_VS_GYM_LEADER_2)       \
	X(MUS_BW_VS_PLASMA_GRUNT_2)     \
	X(MUS_BW_ROUTE_21)              \
	X(MUS_BW_ROUTE_23)              \
	X(MUS_BW_VS_COLRESS)            \
	X(MUS_BW_VS_HUGH)               \
	X(MUS_BW_ROUTE_1)               \
	X(MUS_BW_NACRENE_CITY)          \
	X(MUS_BW_ENTRALINK)             \
	X(MUS_BW_MUSICAL)               \
	X(MUS_BW_OPENING_2)             \
	X(MUS_BW_THE_POKEMON_2)         \
	X(MUS_BW_CHEREN)                \
	X(MUS_BW_BIANCA)                \
	X(MUS_BW_VS_GHETSIS_2)          \
	X(MUS_BW_VS_BW_KYUREM)          \
	X(MUS_BW_SPOTTED_PLASMA_2)      \
	X(MUS_BW_APPEARS_PLASMA_2)      \
	X(MUS_BW_COLRESS)               \
	X(MUS_BW_MARINE_TUBE)           \
	X(MUS_BW2_CHEREN)               \
	X(MUS_BW_VS_PWT_FINAL)          \
	X(MUS_BW_FLOCCESY_RANCH)        \
	X(MUS_BW_ACCUMULA_TOWN)         \
	X(MUS_BW_CASTELIA_SEWER)        \
	X(MUS_BW_OBTAIN_BP)             \
	X(MUS_BW_VS_REGI)               \
	X(MUS_BW_REGI_CHAMBER)          \
	X(MUS_BW_VS_PLASMA_GRUNT_1)     \
	X(MUS_BW_PWT)                   \
	X(MUS_BW_VS_PWT_KANTO_LEADER)   \
	X(MUS_BW_VS_PWT_KANTO_CHAMPION) \
	X(MUS_BW_VS_PWT_JOHTO_LEADER)   \
	X(MUS_BW_VS_PWT_JOHTO_CHAMPION) \
	X(MUS_BW_VS_PWT_HOENN_LEADER)   \
	X(MUS_BW_VS_PWT_HOENN_CHAMPION) \
	X(MUS_BW_VS_PWT_SINNOH_LEADER)  \
	X(MUS_BW_VS_PWT_SINNOH_CHAMPION) \
	X(MUS_BW_VS_IRIS)               \
	X(MUS_BW_HUMILAU_CITY)          \
	X(MUS_BW_VS_N_2)                \
	X(MUS_BW_LENTIMAS_TOWN)         \
	X(MUS_BW_VS_GYM_LEADER_WINNING_2) \
	X(MUS_BW_171)                   \
	X(MUS_BW_REVERSAL_MOUNTAIN)     \
	X(MUS_BW_REVERSAL_MOUNTAIN_2)   \
	X(MUS_BW_SPAWN_ASPERTIA_CITY)   \
	X(MUS_BW_POKEWOOD)              \
	X(MUS_BW_VS_POKEWOOD)           \
	X(MUS_BW_SPOTTED_HARLEQUIN)     \
	X(MUS_BW_VICTORY_PWT)           \
	X(MUS_BW_FINAL_VICTORY_PWT)     \
	X(MUS_BW_FRIGATE_PLASMA)        \
	X(MUS_BW_ZIZONLIN)              \
	X(MUS_BW_GYM_MISTRALTON)        \
	X(MUS_BW_GYM_OPELUCID)          \
	X(MUS_BW_ASSAULT_OPELUCID_CITY) \
	X(MUS_BW_FROZEN_TOWN)           \
	X(MUS_BW_FRIGATE_SORTIE_PLASMA) \
	X(MUS_BW_INFILTRATE_FRIGATE_PLASMA) \
	X(MUS_BW_LOW_HEALTH)            \
	X(MUS_BW_189)                   \
	X(MUS_BW_BLACK_TOWER_ENTRANCE)  \
	X(MUS_BW_BLACK_TOWER)           \
	X(MUS_BW_WHITE_HOLLOW_ENTRANCE) \
	X(MUS_BW_WHITE_HOLLOW)          \
	X(MUS_BW_GYM_VIRBANK)           \
	X(MUS_BW_CASTELIA_GYM)          \
	X(MUS_BW_DRIFTVEIL_GYM)         \
	X(MUS_BW_197)                   \
	X(MUS_BW_GYM_HUMILAU)           \
	X(MUS_BW_LOW_HEALTH_2)          \
	X(MUS_BW_GYM_NIMBASA_RUNAWAY)   \
	X(MUS_BW_GYM_NIMBASA_STAGE)     \
	X(MUS_BW_SPOTTED_BEAUTY)        \
	X(MUS_BW_CREDITS)               \
	X(MUS_BW_CREDITS_END)           \
	X(MUS_BW_PWT_VENUE)
