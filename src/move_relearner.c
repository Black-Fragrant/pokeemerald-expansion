...
static void UIShowMoveList(u8 taskId)
{
    ...
    ShowSelectMovePokemonSummaryScreen_BW(gParties[B_TRAINER_0], gTasks[taskId].tPartyIndex, 1, CB2_InitLearnMoveReturnFromSelectMove, gTasks[taskId].tMove);
    // end bwSummaryScreen
    DestroyTask(taskId);
...
static void Task_MoveRelearner_Quit(u8 taskId)
{
    ...
    if (gInitialSummaryScreenCallback != NULL)
    {
        ...
        */
        ...
        u32 mode;
        if (gRelearnMode == RELEARN_MODE_PSS_PAGE_CONTEST_MOVES)
            mode = SUMMARY_MODE_RELEARNER_CONTEST;
        else
            mode = SUMMARY_MODE_RELEARNER_BATTLE;

        ShowPokemonSummaryScreen_BW(mode, gParties[B_TRAINER_0], gTasks[taskId].tPartyIndex, gPartiesCount[B_TRAINER_0] - 1, gInitialSummaryScreenCallback);
        // end bwSummaryScreen
    }
    else
    {
